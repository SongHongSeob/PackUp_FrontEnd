...
const Header = ({ pageType = 'default' }: HeaderProps) => {
    ...
    const handleMarkAllRead = async  () => {
        ...
        try {
            const res = await fetch('http://localhost:8080/notifications/readAll', {
                method: 'POST',
    ...
    useEffect(() => {
        ...
        const url = `http://localhost:8080/notifications/subscribe?userId=${encodeURIComponent(userId)}`;
        ...
        const fetchUnreadCount  = async () => {
            try {
                const response = await fetch(
                    `http://localhost:8080/notifications/unread_count?userId=${encodeURIComponent(userId)}`,
                    {
                    method: 'GET',
                    credentials: 'include',
        ...
        (async () => {
            try {
                const res = await fetch(
                    `http://localhost:8080/notifications/list?userId=${encodeURIComponent(userId)}`,
    ...
    const handleLogout = async () => {
        try {
            ...
            const response = await fetch('http://localhost:8080/api/lgn/logout', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
